...
- 行情显示：金价 / 沪铜 / 布伦特原油 / 自定义 JSON 数据源（红涨绿跌，10 分钟刷新）
  - **金价**：京东金融·浙商积存金（`api.jdjygold.com`）
  - **沪铜**：东方财富日 K（换环境后东财某实时接口对期货已不放价，固件自动切 K 线端点）
  - **布油**：东方财富实时接口
  - 最多 **6 个行情槽**：网页可增删、设置**轮播间隔（2–60s）**，底部自动轮播（带动画）
...
3. 设备自动重启并弹出 **WiFi 配置窗口** → 填入新环境的 WiFi 与密码 → 浏览器自动打开设备配置页，补填 API Key / 城市。
...
> **送人 / 搬家换网络**：不用重新配置。浏览器打开安装页点「连接设备 · 仅切换 WiFi」→ 选新 WiFi，只更新网络，已保存的天气 API Key、城市/地址、行情、主题全部保留；无电脑时按住 BOOT 上电进入热点（`WeatherClock-xx`）切换亦可。
...
6. **自动定位**：配置页城市栏点「定位」→ 按 IP 自动填城市 + 经纬度；要更精确可到 [高德坐标拾取器](https://lbs.amap.com/tools/picker) 复制坐标填入「经纬度」。
7. **行情**：配置页「行情」卡增删最多 6 个行情槽、设置轮播间隔（2–60s），保存后底部按间隔轮播。
8. **以后想升级固件**：配置页左栏「固件更新」卡片 → 「检查更新」（显示当前版本与 GitHub 最新版本）→ 有新版时点「下载并刷机」，设备自动拉取写入并重启，无需连接电脑。
9. **换环境 / 切换 WiFi**：见上文「送人 / 搬家换网络」，配置全部保留。
...
- 行情：配置页「行情」卡增删槽位、改轮播间隔（2–60s）；金价走京东金融、沪铜走东财日 K、布油走东财实时（数据源集中在 `src/Quote.cpp`）
